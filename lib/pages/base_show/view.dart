...
class BaseShowPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:  Obx((){
        final List<BaseItem> baseList = state.baseList.value;
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      child: Image.asset(baseList[index].url, fit: BoxFit.cover,),
                    ),
                    Container(
                      padding: EdgeInsets.all(15),
                      alignment: Alignment.center,
                      child: Text(baseList[index].text, style: TextStyle(color: AppColors.COLOR_2C3340, fontSize: AppFont.SIZE_28),)
                    )
                  ],
                ),
              );
